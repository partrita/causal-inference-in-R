...
if (!requireNamespace("ggdag", quietly = TRUE) || 
  ...
  # Create fallback functions if packages are not available
  ggdag_geom_dag_label_repel <- function(...) {
    warning("ggdag package not available, using fallback")
    ggplot2::geom_text(...)
  }
  ...
  geom_dag_label_repel_internal <- function(..., seed = 10) {
    ggplot2::geom_text(
      ...
      inherit.aes = FALSE,
      family = getOption("book.base_family", "sans"),
      ...
    )
  }
  
} else {
  # Original code when packages are available
