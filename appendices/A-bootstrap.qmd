...
이 코드는 `r_bootstraps * n_samples`번의 계산이 필요하기 때문에 여기서는 직접 실행하지 않겠지만, 결과는 비슷하게 나타납니다.
부트스트래핑은 위에서 본 잘 정의된 모수적 접근법과 마찬가지로 많은 상황에서 명목상 커버리지를 갖는 신뢰 구간 을 얻게 해줍니다.
이 코드는 `r_bootstraps * n_samples`번의 계산이 필요하기 때문에 여기서는 직접 실행하지 않겠지만, 결과 는 비슷하게 나타납니다.
...
재표본 추출에서 각 관측치를 독립적으로 허용하는 것은, 마치 역확률 가중치 부여에서 샘플들의 가중치를 높이는 것과 비슷하게( [Chapter -@sec-using-ps] 참조), 해당 관측치가 원래 모집단에서 자신과 유사한 다른 관측치들을 대리할 수 있게 해줍니다.
재표본 추출에서 각 관측치를 독립적으로 허용하는 것은, 마치 역확률 가중치 부여에서 샘플들의 가중치를 높이는  것과 비슷하게( [Chapter -@sec-using-ps] 참조), 해당 관측치가 원래 모집단에서 자신과 유사한 다른 관측치들을  대리할 수 있게 해줍니다.
