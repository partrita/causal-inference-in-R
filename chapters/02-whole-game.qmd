...
if (requireNamespace("tipr", quietly = TRUE)) {
  ...
  # Simple message if tipr is not available
  cat("Tipping point analysis requires the tipr package which is not available.\n")
  cat("The analysis would show how strong an unmeasured confounder would need to be\n")
  cat("to change our conclusions about the effect of bed nets on malaria risk.\n")
  
  # Create a placeholder tipping_points for the text below
  tipping_points <- data.frame(
    confounder_outcome_effect = c(-0.15, -0.03),
    exposure_confounder_effect = c(1, 5)
  )
}
```
...
In that case, the confounder-malaria relationship only needs to be `r round(tipping_points$confounder_outcome_effect[[5]], digits = 1)`.
