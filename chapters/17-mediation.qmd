...
그러나 많은 연구 질문에서는 "왜(why)?" 또는 "어떻게(how)?"라는 질문에 답하고 싶습니다.
예를 들어, 엑스트라 매직 아워가 대기 시간에 영향을 미친다면, 그 효과의 얼마가 방문객 수의 변화를 *통해(through)* 발생하는 것일까요?

이런 질문에 답하기 위한 분석 방법이 바로 **인과 매개 분석(causal mediation analysis)**입니다.
매개 분석은 처치 효과를 두 가지 성분으로 분리합니다:

- **직접 효과(direct effect)**: 매개변수를 거치지 않고 노출이 결과에 직접적으로 미치는 효과
- **간접 효과(indirect effect)**: 매개변수를 통해 노출이 결과에 미치는 효과
...
#| warning: false
#| fig-cap: >
#|   매개 분석의 기본 구조를 보여주는 DAG. 노출(X)은 매개변수(M)를 통해 간접적으로
...
인과 매개 분석을 유효하게 수행하기 위해서는 네 가지 형태의 교란이 없어야 합니다:
...
상호작용이 있을 때의 자연 직접 효과와 간접 효과는 반사실적 시뮬레이션을 사용하여 계산할 수 있습니다.
이 경우 G-공식(@sec-g-comp) 접근법이 자연스럽게 확장됩니다.
