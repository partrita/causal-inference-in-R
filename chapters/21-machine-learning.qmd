...
이 장에서는 머신러닝과 인과 추론을 어떻게 결합할 수 있는지 탐색합니다:
1. 예측과 인과 추론의 근본적인 차이를 다시 살펴보기
2. 이중 로버스트 추정량(@sec-dr)과 머신러닝을 결합하기
3. 교차 적합(cross-fitting)의 중요성

## 예측과 인과 추론, 다시 보기 (Prediction and causal inference, again)

@sec-casual-to-causal 에서 예측과 인과 추론의 차이를 처음 소개했습니다.
여기서 이 차이를 머신러닝의 맥락에서 다시 살펴봅니다.

### 예측 정확도 ≠ 인과 추론

머신러닝 모델의 목표는 주어진 입력에서 결과를 최대한 정확히 예측하는 것입니다.
반면 인과 추론의 목표는 개입의 효과를 편향 없이 추정하는 것입니다.
이 두 가지는 근본적으로 다른 목표입니다.
...
예를 들어 LASSO는 불필요한 변수의 계수를 0으로 만들어 모델을 단순화합니다.

그러나 인과 추론에서 중요한 교란 요인의 계수가 정규화에 의해 0으로 설정된다면, 이는 편향을 유발합니다.
따라서 머신러닝을 인과 추론에 적용할 때는 각별한 주의가 필요합니다.
:::

## 증강된 성향 점수 (Augmented propensity scores)

@sec-dr 에서 소개한 AIPW는 성향 점수와 결과 모델 모두를 사용합니다.
...
@sec-dr 에서 소개한 TMLE은 머신러닝과 결합했을 때 특히 강력합니다.
...
교차 적합이 필요한 이유: 같은 데이터로 성향 점수/결과 모델을 학습하고 AIPW를 계산하면, 과적합으로 인한 "과잉 적합 편향(overfitting bias)"이 발생할 수 있습니다.
...
1. 머신러닝으로 교란 요인으로부터 노출을 예측하고 잔차 계산: $\tilde{X} = X - \hat{E}[X \mid C]$
2. 머신러닝으로 교란 요인으로부터 결과를 예측하고 잔차 계산: $\tilde{Y} = Y - \hat{E}[Y \mid C]$
3. 잔차에 대한 선형 회귀: $\tilde{Y} \sim \tilde{X}$
...
2. **함수 형태 불확실성**: 올바른 모델 형태를 모를 때
3. **이질적 효과 추정**: CATE 추정에서 특히 유용
