...
인과 추론에서 다른 점은 이 과정을 인과적 질문 및 목표 시험 모방(target trial emulation)과 어떻게 연결하느냐 하는 것입니다.
또한 이 기회를 빌려 우리 데이터가 인과적 가정을 얼마나 잘 충족하는지 더 잘 이해할 수 있습니다. 비록 @sec-quartets에서 보았듯이, 데이터만으로는 우리가 옳은지 그른지 결코 알 수 없지만 말입니다.
...
@sec-assump와 @sec-quartets에서 보았듯이, 데이터만으로는 인과 추론에 필요한 검증 불가능한 가정 문제를 해결할 수 없습니다.
...
교환 가능성에 대한 데이터 확인은 @sec-eval-ps-model에서 더 나은 도구를 써서 이 가정을 조사할 때까지 미루겠습니다.
