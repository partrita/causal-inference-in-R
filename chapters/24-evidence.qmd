...
# 연구 표본에서의 가중치 계산
study_sample_with_weights <- sampling_model |>
  augment(type.predict = "response", data = study_sample) |>
  mutate(
    # 운송 가중치: (1 - P(S=1)) / P(S=1)
    transport_weight = (1 - .fitted) / .fitted
  )
...
transported_result <- ps_study |>
  augment(type.predict = "response",
          data = study_sample_with_weights) |>
  mutate(
    w_combined = wt_ate(.fitted, park_extra_magic_morning) * transport_weight
  ) |>
  lm(wait_minutes_posted_avg ~ park_extra_magic_morning,
