...
이 장에서는 종단(longitudinal) 데이터에서 인과 추론을 수행하기 위한 핵심 개념과 방법을 소개합니다:

1. **추적 관찰 소실(loss to follow-up)**이 어떻게 편향을 유발할 수 있는지
2. **시간 가변적 교란 요인(time-varying confounders)**을 어떻게 올바르게 처리하는지

## 추적 관찰 소실 (Loss to follow-up)

종단 연구에서는 연구 기간 동안 일부 참가자가 연구에서 이탈(drop out)하는 경우가 발생합니다.
이를 **추적 관찰 소실(loss to follow-up, LTFU)**이라고 합니다.

추적 관찰 소실이 단순히 무작위적으로 발생한다면(완전 무작위 결측, MCAR) 편향을 유발하지 않습니다.
그러나 현실에서는 종종 이탈 여부가 노출이나 결과와 관련된 요인에 의해 결정됩니다.
이 경우 **선택 편향(selection bias)**이 발생하며, 이를 **정보적 검열(informative censoring)**이라고도 부릅니다.
...
추적 관찰 소실로 인한 선택 편향은 **역확률 검열 가중치(Inverse Probability of Censoring Weights, IPCW)**를 사용하여 교정할 수 있습니다.
이는 이탈하지 않을 확률의 역수를 각 관측치에 가중치로 부여하여, 이탈하지 않은 참가자들로 이탈한 참가자들의 정보를 복원하는 방식입니다.
...
시간 가변적 교란 요인은 시간이 지남에 따라 변하는 교란 요인으로, 이전의 노출에도 영향을 받는다는 점이 핵심입니다.
...
위의 DAG에서 `L_t`를 표준 회귀 방법으로 조정하면 **충돌체 편향(collider bias)**이 발생합니다.
`L_t`는 `X_{t-1}`의 결과이자 `X_t`와 `Y`의 원인이므로, `L_t`를 통제하면 `X_{t-1}`과 `Y` 사이의 경로를 열어버리기 때문입니다.

이 문제를 해결하기 위해 Robins(1986)가 개발한 **한계 구조 모델(Marginal Structural Model, MSM)**을 사용합니다.

### 한계 구조 모델과 시간 가변적 IPTW

한계 구조 모델은 시간 가변적 처치와 교란을 다루기 위한 강력한 도구입니다.
...
한계 구조 모델에서 핵심은 결과 모델(**MSM**)에서 시간 가변적 교란 요인(`L_t`)을 포함하지 않는다는 점입니다.
교란은 이미 IPTW 가중치를 통해 처리되었기 때문입니다.
MSM에 교란 요인을 다시 포함시키면 가중치 부여의 효과가 상쇄됩니다.

이는 단면 분석에서 IPTW를 사용할 때와 마찬가지 원리이지만(@sec-outcome-model), 종단 데이터에서는 각 시점의 가중치를 곱해서 사용한다는 차이가 있습니다.
:::
