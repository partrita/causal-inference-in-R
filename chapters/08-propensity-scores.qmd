...
첫째, @fig-dag-close-paths-1 에서처럼 교란 요인들과 노출 사이의 관계를 모델링할 수 있습니다. 노출될 확률인 **성향 점수(propensity score)**를 사용하는 일련의 기술들을 통해 이를 수행할 수 있습니다. 둘째, @fig-dag-close-paths-2 에서처럼 교란 요인들과 결과 사이의 관계를 모델링할 수 있습니다. **결과 모델(outcome model)**을 사용하는 일련의 기술들을 통해 이를 수행할 수 있습니다. 우리는 @sec-strat-outcome 에서 결과 모델을 보았고, @sec-g-comp 에서 또 다른 접근 방식을 보게 될 것입니다. 성향 점수와 결과 모델 접근 방식 모두 올바른 DAG를 가지고 관계를 올바르게 모델링했다면 정답을 얻을 수 있게 해줍니다. 대안적으로, @fig-dag-close-paths-2 에서처럼 두 세트의 관계를 모두 모델링할 수 있습니다. **이중 로버스트(doubly robust)** 추정량이라고 불리는 일련의 기술들을 통해 이를 수행할 수 있으며, 이에 대해서는 @sec-dr 및 @sec-causal-ml 에서 다시 다룰 것입니다.

다음 몇 장에 걸쳐 우리는 @fig-dag-close-paths-1 을 통해 경로를 차단하는 데 사용할 수 있는 기술군인 성향 점수를 다룰 것입니다. 먼저, 디즈니가 엑스트라 매직 아워 배정을 무작위로 했다면 어떤 모습일지 생각해 봅시다. 각 날짜가 엑스트라 매직 아워에 배정될 확률이 0.5라고 가정해 봅시다. 그러면 약 절반의 날짜는 매직 아워가 있고 나머지 절반은 없을 것입니다. 이 경우 @fig-dag-close-paths-1 에 강조된 화살표들은 존재하지 않겠지만, @fig-dag-close-paths-2 에 강조된 화살표들은 존재할 것입니다: 우리는 노출(그날 엑스트라 매직 아워가 있는지 여부)에 개입한 것이지, 결과(게시된 대기 시간)에 개입한 것이 아니기 때문입니다. 그날의 과거 최고 기온, 공원 폐쇄 시간, 티켓 시즌은 여전히 게시된 대기 시간에 영향을 미치지만, 엑스트라 매직 아워에는 영향을 미치지 않습니다. 여기서 노출 확률 — 성향 점수 — 은 매일 0.5입니다. 다시 말해, 실험에서는 성향 점수를 알고 있습니다. 주어진 날짜는 `rbinom(1, 1, 0.5)`를 통해 무작위로 배정됩니다.

디즈니가 더 복잡한 실험을 수행했다고 가정해 봅시다: 그들은 *DAG의 세 가지 변수의 수준 내에서* 노출을 무작위로 배정했습니다. 예를 들어, 모든 날짜에 기본 확률 0.5를 부여했다고 합시다. 이 실험에서 과거 기온이 화씨 80도 이상이면 노출 확률이 0.1 감소하고, 공원 폐쇄 시간이 오후 10시 이전이면 확률이 0.2 증가하며, 성수기 티켓 시즌이면 확률이 0.3 감소합니다. 과거 기온이 86도이고 성수기 티켓 시즌에 오후 9시에 폐장한 날의 경우, 그날 아침에 엑스트라 매직 아워가 무작위로 배정될 확률은 $0.5 - 0.1 + 0.2 - 0.25 = 0.35$, 즉 `rbinom(1, 1, 0.35)`가 됩니다. 이 설계는 여전히 무작위 실험이지만, 공변량의 수준 내에서 무작위화된 것입니다. 이는 무작위 배정되지 않은 데이터에서 우리가 가정해야 하는 것과 매우 유사한 조건부 교환 가능성(conditional exchangeability)을 제공합니다: 날짜들은 공변량의 수준 내에서 교환 가능합니다.

하지만 우리의 경우, 이러한 확률들을 알 수 없습니다. 만약 이 무작위 배정되지 않은 데이터에서 "숨겨진 실험"을 찾을 수 있다면 어떨까요? 누군가는 특정 날짜에 엑스트라 매직 아워를 넣기로 결정했다는 점을 기억하십시오. 그 의사 결정 과정은 무엇이었을까요? 만약 우리가 도메인 지식(처치 배정에 영향을 미치면서 결과에도 영향을 주는 요인들이 무엇인지 이해하기 위해)과 통계학(이러한 조건부 확률을 추정하기 위해)을 결합하여 사용할 수 있다면, 아마도 그 확률들을 사용하여 교환 가능성을 달성하고 편향되지 않은 인과 효과를 계산할 수 있을 것입니다. @rosenbaum1983central 은 @sec-assump 에서 논의한 가정들이 충족되는 한, 관찰 연구에서 성향 점수에 대해 조건부화하는 것이 노출 효과의 편향되지 않은 추정치로 이어질 수 있음을 보여주었습니다.

## 성향 점수 모델 구축하기 {#sec-building-models}

성향 점수를 추정하는 방법은 많습니다. 이진 노출의 경우 대개 로지스틱 회귀가 사용되지만, 다른 접근 방식들도 사용됩니다. 로지스틱 회귀로부터 얻은 추정치는 인과적 관점에서 해석하기 어려울 수 있지만(@sec-binary 에서 더 자세히 다룰 것입니다), 잘 보정된 확률을 예측하는 데는 탁월합니다. 노출을 예측값으로 하고 교란 요인들을 공변량으로 하는 로지스틱 회귀는 성향 점수를 시작하기에 좋은 지점입니다.

아래는 로지스틱 회귀를 사용하여 성향 점수 모델을 적합시키기 위해 `glm()`을 사용하는 가상 코드입니다. 첫 번째 인자는 모델로, 왼쪽에는 노출이 있고 오른쪽에는 교란 요인들이 있습니다. `data` 인자는 데이터 프레임을 받고, `family = binomial()` 인자는 모델이 (다른 일반화 선형 모델이 아닌) 로지스틱 회귀를 사용하여 적합되어야 함을 나타냅니다 (성향 점수 모델링에서 다른 연결 함수들이 사용되기도 하지만요). 여러분은 이와 같은 모델을 이전에 적합시켜 본 적이 있을 것이나, 핵심적인 세부 사항은 우리가 (결과와 관련된 무언가가 아닌!) 노출 확률을 예측하고 있다는 점과 예측 변수들이 DAG로부터 결정된 교란 요인들이라는 점입니다. @sec-ci-rct 에서 보았듯이, 만약 노출과는 관련이 없지만 결과의 예측 변수인 요인들이 있다면, 정밀도를 위해 이들도 포함하는 것이 좋습니다.
...
절단과 트리밍은 캘리퍼 사용과 마찬가지로 우리가 추론을 이끌어내는 인구 집단을 변화시킬 수 있습니다. 우리는 @sec-estimands 에서 이를 더 조사할 것입니다.
