...
이 장에서는 DiD를 더 깊이 탐구하고, 현대의 확장된 방법들과 관련 접근법인 **합성 대조군(synthetic controls)**을 소개합니다.

이중 차분법은 다음 상황에서 특히 유용합니다:
- 처치를 받은 그룹과 받지 않은 그룹이 있을 때
- 처치 전후의 데이터가 있을 때
...
DiD의 핵심 아이디어는 처치군에서 관찰된 변화에서 처치가 없었을 경우의 변화를 제외하는 것입니다.
대조군의 변화가 없었을 경우의 반사실적 변화를 대리하는 역할을 합니다.
...
이벤트 스터디에서 처치 전 지점들의 추정치가 0에 가까우면 평행 추세 가정에 대한 지지 증거입니다.

### 이종적 처치 시점 문제 (Staggered adoption)

최근 계량경제학 연구에서 TWFE 모델의 중요한 한계가 밝혀졌습니다.
서로 다른 시점에 처치를 받는 **이종 처치 시점(staggered adoption)** 설계에서 TWFE 추정치는 음수 가중치가 부여된 처치 효과의 혼합이 될 수 있어 해석이 어렵습니다.
...
**합성 대조군(Synthetic Control Method, SCM)**은 Abadie et al.(2010)이 개발한 방법으로, 처치를 받은 단일 단위(예: 한 국가나 지역)에 대한 반사실적 결과를 대조군들의 가중 조합으로 구성합니다.

이 방법은 처치를 받지 않은 집단이 처치군의 좋은 비교 대상이 될 수 없는 경우 특히 유용합니다.
...
합성 대조군 방법의 처치 효과는 처치 후 기간에서 실제 결과와 합성 대조군의 차이로 계산됩니다.
추론을 위해서는 **치환 검정(permutation tests)** 또는 **플라세보 검정(placebo tests)**을 사용합니다.
...
두 방법 모두 시계열 데이터에서 인과 효과를 추정하는 강력한 도구이며, 연구 설계와 데이터 구조에 따라 적합한 방법을 선택해야 합니다.
:::
