...
잠시 멈추어 지금까지 이 책에서 본 인과 분석의 전형적인 목표를 정리해 봅시다: 연구에 참여한 *모든 사람*이 노출되었을 때 일어날 일과 *아무도* 노출되지 않았을 때 일어날 일을 추정하는 것입니다.
이를 위해 우리는 교란 요인의 균형이 잡힌 가상 인구(pseudopopulations)를 생성하는 가중치 부여 기술을 사용해 왔으며, 이는 다시 한계 결과 모델(marginal outcome models)에서 편향되지 않은 인과 효과 추정치를 제공합니다.
가중치 부여에 대한 한 가지 대안적인 접근 방식은 **모수적 G-공식(parametric G-formula)**이라고 불리며, 일반적으로 다음의 4단계를 통해 실행됩니다:

1.  적절한 시간 순서의 DAG를 그립니다 (@sec-dags 에서 설명한 대로).

2.  기준점(baseline) 이후의 각 시점에 대해, DAG에서 이전에 측정된 변수들을 기반으로 각 변수의 값을 예측하는 모수적 모델(parametric model)을 결정합니다.
    이들은 종종 연속형 변수의 경우 선형 모델이거나 이진 변수의 경우 로지스틱 회귀 모델입니다.

3.  기준점에서의 관찰된 데이터 분포로부터 추출된 표본으로 시작하여, 2단계의 모델에 따라 이후의 모든 변수 값을 생성합니다(즉, *몬테카를로 시뮬레이션*을 수행합니다).
    이때 한 가지 핵심적인 수정을 가합니다: 비교하고자 하는 각 노출 체계(예: 모두 노출됨 vs 모두 노출되지 않음)에 대해 노출 변수를 그에 맞게 할당합니다(즉, 시뮬레이션이 노출 변수의 값을 정하게 두지 않습니다).
...
실제로, 그것이 바로 이 책에서 설명하는 현실 세계의 인과 메커니즘에 몬테카를로 시뮬레이션이 매우 유용하게 쓰이는 이유입니다!
:::

## 엑스트라 매직 아워 예시 다시 보기

@sec-outcome-model 에서 우리가 아침 엑스트라 매직 아워가 오전 9시와 10시 사이의 '세븐 드워프' 놀이기구 평균 게시 대기 시간에 미치는 영향을 추정했던 것을 기억해 보십시오.
이를 위해 우리는 교란 요인인 `park_ticket_season`, `park_close`, `park_temperature_high`를 사용하여 노출(`park_extra_magic_morning`)에 대한 성향 점수 모델을 적합시켰습니다.
결과적으로 이 성향 점수들은 결과 모델을 위한 회귀 가중치로 변환되었으며, 엑스트라 매직 아워가 있는 것이 오전 9시에서 10시 사이의 평균 게시 대기 시간에 미치는 기대 영향이 6.2분이라는 결론을 내렸습니다.

이제 우리는 g-공식 접근법을 채택하여 이 분석을 재현해 볼 것입니다.
위에서 설명한 4단계를 따라, 이 질문과 관련된 시간 순서의 DAG를 다시 살펴보는 것부터 시작하겠습니다.
...
두 번째 단계는 기준점이 아닌 각 변수에 대해, DAG에서 이전에 측정된 변수들을 기반으로 하는 모수적 모델을 지정하는 것입니다.
이 예시는 이전 특성들에 의해 영향을 받는 변수가 두 개(`park_extra_magic_morning`과 `wait_minutes_posted_avg`)뿐이므로 간단합니다.
이 두 변수에 대한 적절한 모델이 아래와 같은 단순한 로지스틱 및 선형 모델이라고 가정해 봅시다.
참고로, 아직 노출(`park_extra_magic_morning`)에 대한 모델을 사용하지는 않겠지만, 다음 섹션(@sec-dynamic)에서 보게 될 패턴의 중요한 부분이기 때문에 이 단계를 포함시켰습니다.
...
앞서 언급했듯이, G-공식의 핵심적인 장점 중 하나는 연속형 노출을 처리할 수 있는 능력입니다. 연속형 노출 상황은 IP 가중치 부여가 불안정한 추정치를 낳을 수 있는 상황입니다.
여기서는 이것이 어떻게 수행되는지 보여주기 위해 @sec-continuous-exposures 의 예시를 짧게 반복합니다.
패턴을 확장하기 위해, 신뢰 구간이 어떻게 계산되는지 보여주기 위해 이 기술의 실행 과정을 부트스트랩으로 감쌀 것입니다.
...
이제 이 모든 것을 하나로 합쳐서 단일 점 추정치를 구해 봅시다.
...
우리는 중소 규모의 데이터셋에서 모수적 G-공식을 사용할 때 해결해야 할 흔한 과제를 강조하기 위해 이 경고를 그대로 남겨두었습니다.
