...
# ceiling(time)이 0이 될 수 있으므로 pmax(1, ...) 로 최솟값을 1로 보장
make_long_rows <- function(id, treatment, age_group, severity, time_int, event) {
...
survival_long <- survival_sim |>
  ...
  mutate(
    rows = purrr::pmap(
      list(id, treatment, age_group, severity, time_int, event),
      make_long_rows
    )
  ) |>
  tidyr::unnest(rows) |>
