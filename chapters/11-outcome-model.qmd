...
예를 들어 @sec-using-ps에서 했던 것처럼 매칭을 다시 수행하고, 다음과 같이 `matched_data`라는 데이터셋에 매칭된 관측치를 추출할 수 있습니다.
...
그러나 로지스틱 회귀를 사용할 때, @sec-g-comp장에서 보겠지만 예측 확률(predicted probabilities)을 사용하여 위험 차이와 위험비를 계산할 수도 있습니다.
