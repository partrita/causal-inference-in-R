...
인과 관계에 대한 질문에 답하는 것은 과학적 및 비즈니스 목적 모두에 중요하지만, 무작위 임상 시험(RCT)이나 A/B 테스트와 같은 기술이 항상 실용적이거나 성공적인 것은 아닙니다.
이 책의 도구들은 독자들이 R 프로그래밍 언어를 사용하여 관측 데이터(observational data)로 더 나은 인과 추론을 할 수 있도록 도울 것입니다.
이 책을 마칠 때쯤이면 다음을 할 수 있기를 바랍니다:

1.  더 나은 인과 관계 질문 던지기.
2.  인과 추론에 필요한 가정 이해하기.
3.  추론을 하고자 하는 대상 인구(target population) 식별하기.
4.  인과 모델을 적합시키고 문제점 확인하기.
5.  사용된 기술이 불완전할 수 있는 상황에서 민감도 분석(sensitivity analysis) 수행하기.

이 책은 학술 연구자와 데이터 과학자 모두를 위한 것입니다.
이러한 환경에 따라 질문은 다를 수 있지만, 많은 기술은 동일합니다. 인과 추론은 암에 대한 질문을 던지는 것만큼이나 클릭 수에 대한 질문을 던지는 데에도 유용합니다.
의학, 경제학, 기술 및 기타 분야의 다양한 사례를 혼합하여 사용하여 명확한 인과 관계 질문과 가정에 대해 투명하게 공개하려는 의지가 필요함을 보여줍니다.

이 책에서 많은 것을 배우겠지만, 아이러니하게도 인과 추론의 가장 좋은 도구 중 하나인 무작위 시험을 수행하는 방법에 대해서는 많이 배우지 않을 것입니다.
무작위 시험과 그 사촌 격인 A/B 테스트(기술 분야의 표준)는 유효한 추론을 위해 우리가 해야 하는 많은 가정을 완화해주기 때문에 매우 강력합니다.
또한 설계가 충분히 복잡하여 자체적인 학습 리소스가 필요할 정도입니다.
대신, 우리는 일반적으로 무작위 배정의 이점을 누릴 수 없는 관측 데이터에 집중할 것입니다.
무작위 배정 기술에 관심이 있더라도 이 리소스를 바로 덮지 마세요. 관측 데이터를 위해 설계된 많은 인과 추론 기술은 무작위 분석도 개선할 수 있습니다.

우리는 독자인 여러분에 대해 몇 가지 가정을 하고 있습니다:
...
이 주제에 대한 자세한 내용은 [이 Tidyverse 블로그 포스트](https://www.tidyverse.org/blog/2023/04/base-vs-magrittr-pipe/)를 참조하세요.
...
이 주제에 대한 자세한 내용은 [R for Data Science의 반복(iteration) 장](https://r4ds.hadley.nz/iteration.html)을 참조하세요.
